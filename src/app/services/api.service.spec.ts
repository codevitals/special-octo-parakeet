...
describe('ApiService', () => {
  ...
  it('login should return user Id if auth success', (done) => {
    const username = 'username';
    const password = 'password';
    const userId = 1;
  ...
  it('login should return error message if auth fails', (done) => {
    ...
    const loginRequest = httpMock.expectOne({ method: 'POST', url: _baseUrl });
    loginRequest.flush(expectedErrorResponse, { status: 401, statusText: 'Unauthorized' });
  ...
  it('getDiseasesList should return suffer list', (done) => {
