...
export class LoginComponent implements OnInit {
	...
	ngOnInit() {
		...
		this.loginForm = new FormGroup({
			userName: new FormControl('', [Validators.required, Validators.minLength(3), Validators.maxLength(20), Validators.pattern(/^[a-zA-Z0-9]+$/)]),
      		password: new FormControl('', [Validators.required, Validators.minLength(8), Validators.maxLength(20), Validators.pattern(/^[-@.\/#&!$+\w\s]*$/)])
		});
	}
	...
	doLogin() {
		...
		try {
			...
			this.dataService.authenticateUser(userName, password).subscribe(data => {
				if (data == true) {
